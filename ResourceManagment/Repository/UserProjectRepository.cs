    ...
    public class UserProjectRepository : IUserProjectRepository
    {
        ...
        IList<UserProjectResponse> IUserProjectRepository.GetUserProject(int Depid)
        {
            ...
            foreach (var item in upList)
            {
                ...
                { 
                    id=item.id,UserName = item.UserName ,userid=item.userid,ProjectName=item.ProjectName,projectid=item.projectid,
                    CordinatorName=item.CordinatorName,pcid=item.pcid,
                    ManagerName=item.ManagerName,pmid=item.pmid,Avalibiltty=item.Avalibiltty,
